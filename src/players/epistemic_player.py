...
class EpistemicPlayer(LogicPlayer):
    ...
    def update_knowledge(self, drawing_player, giving_player, current_players, discarded, card_types):
        ...
        self.add_epistemic_knowledge(drawing_player, giving_player, current_players, discarded, card_types)
    ...
    def add_epistemic_knowledge(self, drawing_player, giving_player, current_players, discarded, card_types):
        if not discarded and self.id != drawing_player.id and self.id != giving_player.id and giving_player in current_players:
            self.knowledge.append(Or([K(giving_player.id, Atom(drawing_player.id, card)) for card in card_types]))
            self.knowledge.append(Or([K(drawing_player.id, Neg(Atom(giving_player.id, card))) for card in card_types]))
